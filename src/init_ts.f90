SUBROUTINE init_ts(gw_ini, sw_ini, auto_advance)
	...
	LOGICAL, INTENT(IN) :: auto_advance ! auto advances by default

	INTEGER(INT16) :: idx
	CHARACTER(LEN=256) :: strbuffer

	REAL(REAL64), INTENT(IN), DIMENSION(:) :: gw_ini, sw_ini
	...
	time% Lstart = time% Gstart + timedelta(seconds = time% Gdt% total_seconds() * time% Gts-1)
