...
function help() {
    ...
    echo ' -d: compile program for debugging'
    echo ' -h: print this help'
    exit 0
}

while getopts ':pndgh' opt; do
    case $opt in
        p)
            cd src/
            ...
            gfortran -c Mtiming.f90 Mpaths.f90 Mlogger.f90 Muz.f90 Msolver.f90 Mstorages.f90 model.f90 \
                ...
                -L/usr/local/lib/ -lfgsl -lgsl -lgslcblas -lm -lgomp -lyaml-interface -lyaml-read -lyaml-wrapper -lyaml-cpp -ldatetime \
            ...
            if [ $? -eq 0 ]; then
                ...
                (f2py3 --verbose -c -m gwswex_wrapper --build-dir f2py_scratch --fcompiler=gnu95 --f90flags='-fopenmp -march=znver2 -mtune=znver2  -ftree-vectorize -frecursive' --opt='-O3' \
                    ...
                    if [ $? -eq 0 ]; then
                        ...
                        rm -rf *.pyf f2py_scratch 2>/dev/null
        ...
        n)
            ...
            gfortran -c Mtiming.f90 Mpaths.f90 Mlogger.f90 Muz.f90 Msolver.f90 Mstorages.f90 model.f90 \
                -I/usr/local/include -I/usr/local/include/yaml-fortran -I/usr/local/include/fgsl/ \
                -L/usr/local/lib/ -lfgsl -lgsl -lgslcblas -lm -lyaml-interface -lyaml-read -lyaml-wrapper -lyaml-cpp -ldatetime \
        ...
        g)
            ...
            if [ $? -eq 0 ]; then
                ...
                export LDFLAGS=-Wl,-rpath=../lib/
                export NPY_DISTUTILS_APPEND_FLAGS=1
                (f2py3 --verbose -c -m --debug-capi gwswex_wrapper --build-dir f2py_scratch --fcompiler=gnu95 --f90flags='-march=znver2 -mtune=znver2' --opt='-O3' \
                    -I/usr/local/include -I/usr/local/include/yaml-fortran -I/usr/local/include/fgsl/ \
                    -L/usr/local/lib/ -lfgsl -lgsl -lgslcblas -lm -lgomp -lyaml-interface -lyaml-read -lyaml-wrapper -lyaml-cpp -ldatetime \
                    Mtiming.o Mpaths.o Mlogger.o Muz.o Msolver.o Mstorages.o model.o GWSWEX_wrapper.f90) >>../build.log 2>&1
                    if [ $? -eq 0 ]; then
