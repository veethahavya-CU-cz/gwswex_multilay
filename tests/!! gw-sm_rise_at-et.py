...
p = np.full((elems,Gnts+1), 2*(1e-3/3600)) #mm/h
p[:,0:500] = 1.66*(1e-3/3600)
p[:,500:750] = 0*(1e-3/3600)
p[:,1000:1250] = 0*(1e-3/3600)
p[:,1000:1250] = 0*(1e-3/3600)
# p[:,1750:2000] = 0*(1e-3/3600)
p[:,2100:Gnts] = 0*(1e-3/3600)
et = np.full((elems,Gnts+1), 0.66*(1e-3/3600))

isactive = np.full((nlay, elems), 1, dtype=int, order='F')
gw_ini = np.array(bot[2] + 12.5, dtype=np.float64, order='F')
