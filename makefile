...
$(OBJS):
	cd $(BUILD_PATH) && \
		$(FC) -c $(SRC_MODULES) $(INCLUDES) $(LD_FLAGS) $(FFLAGS)

$(TARGET_LIB_F2PY): $(OBJS)
	cd $(BUILD_PATH) && \
		$(F2PY) $(F2PYFLAGS) -c -m $(TARGET_BASENAME_F2PY) $(INCLUDES) $(LD_FLAGS) $(OBJS) $(SRC_WRAPPER)
...
$(TARGET_LIB_PARALLEL): $(OBJS) $(SRC_WRAPPER)
	cd $(BUILD_PATH) && \
		$(FC) -shared -o $@ $(OBJ_NAMES) $(SRC_WRAPPER) $(INCLUDES) $(LD_FLAGS) $(FFLAGS)

$(TARGET_EXE_PARALLEL): $(OBJS) $(SRC)
	cd $(BUILD_PATH) && \
		$(FC) -o $@ $(OBJS) $(SRC) $(INCLUDES) $(LIB_PATHS) $(LD_FLAGS) $(FFLAGS)